...
from ..models.base import User, Category, Product, ProductBundle, Order, Deposit, PaymentMethod, KYCRequest, Notification, Setting, AdminActivity
...
def is_admin_user(identity):
    # في هذا الإعداد البسيط، نتحقق من اسم المستخدم وكلمة المرور
...
def admin_categories():
    ...
    if request.method == "GET":
        ...
        return jsonify([{"id": c.id, "name": c.name, "description": c.description, "image": c.image, "is_active": c.is_active, "order": c.order} for c in categories])
    else:
        data = request.get_json()
        cat = Category(
            name=data.get("name"),
...
def admin_products():
    ...
    if request.method == "GET":
        ...
        return jsonify([{
            ...
            "image": p.image, "product_type": p.product_type, "base_quantity": p.base_quantity,
            "base_price": p.base_price, "unit_name": p.unit_name, "input_type": p.input_type,
            "custom_input_label": p.custom_input_label, "stock": p.stock, "is_bundle": p.is_bundle, "is_active": p.is_active,
        } for p in products])
    else:
        data = request.get_json()
        product = Product(
...
def admin_bundles(product_id):
    ...
    if request.method == "GET":
        ...
        return jsonify([{"id": b.id, "name": b.name, "quantity": b.quantity, "price_usd": b.price_usd, "is_active": b.is_active} for b in bundles])
    else:
        data = request.get_json()
...
def admin_orders():
    ...
    return jsonify([{
        ...
        "total_price": o.total_price, "status": o.status, "delivery_data": o.delivery_data,
...
def admin_deposits():
    ...
    return jsonify([{
        "id": d.id, "user_id": d.user_id, "amount": d.amount, "method": d.method,
        "proof_image": d.proof_image, "status": d.status, "transaction_id": d.transaction_id,
...
def admin_approve_deposit(deposit_id):
    ...
    if deposit.status == "pending":
        ...
        if user:
            ...
            txn = Transaction(user_id=user.id, type="deposit", amount=deposit.amount, balance_after=user.balance, reference_type="deposit", reference_id=deposit.id)
            db.session.add(txn)
...
def admin_kyc():
    ...
    return jsonify([{
        "id": k.id, "user_id": k.user_id, "full_name": k.full_name, "phone": k.phone,
        "id_front_image": k.id_front_image, "id_back_image": k.id_back_image,
        "status": k.status, "submitted_at": k.submitted_at.isoformat() if k.submitted_at else None,
    } for k in kycs])
...
def admin_payment_methods():
    ...
    if request.method == "GET":
        ...
        return jsonify([{
            "id": m.id, "name": m.name, "description": m.description, "account": m.account,
            "account_name": m.account_name, "icon": m.icon, "min_amount": m.min_amount,
